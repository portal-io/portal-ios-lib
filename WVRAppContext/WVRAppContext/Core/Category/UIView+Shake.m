...
@property (nonatomic, assign) BOOL isShaking_suspend;
...
- (void)_doAnimation:(CGFloat)direction {
    ...
    // UIViewAnimationOptionAllowUserInteraction 动画的同时可响应交互事件
    [UIView animateWithDuration:iterationDuration delay:0 options:UIViewAnimationOptionAllowUserInteraction animations:^{
...
- (void)suspend {
    ...
    if (shakeInfo.shaking) {
        ...
        shakeInfo.isShaking_suspend              = YES;
        self.transform                           = shakeInfo.baseTransform;
    }
}
...
- (void)resume {
    ...
    if (shakeInfo.isShaking_suspend) {
        
        shakeInfo.isShaking_suspend = NO;
        
        [self shakeWithOptions:shakeInfo.options
                         force:shakeInfo.force
                      duration:shakeInfo.duration
             iterationDuration:shakeInfo.iterationDuration
             completionHandler:shakeInfo.completionHandler];
    }
}
