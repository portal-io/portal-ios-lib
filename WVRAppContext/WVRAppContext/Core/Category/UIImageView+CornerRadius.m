...
- (void)wvr_setCornerImage:(NSString *)url {
...
- (instancetype)initWithRoundingRectImageView {
    self = [super init];
    if (self) {
        [self wvr_cornerRadiusRoundingRect];
    }
    return self;
}
...
- (instancetype)initWithCornerRadiusAdvance:(CGFloat)cornerRadius rectCornerType:(UIRectCorner)rectCornerType {
    self = [super init];
    if (self) {
        [self wvr_cornerRadiusAdvance:cornerRadius rectCornerType:rectCornerType];
    }
    return self;
}
 ...
 */
- (void)wvr_cornerRadiusWithImage:(UIImage *)image cornerRadius:(CGFloat)cornerRadius rectCornerType:(UIRectCorner)rectCornerType {
 ...
 */
- (void)wvr_cornerRadiusWithImage:(UIImage *)image cornerRadius:(CGFloat)cornerRadius rectCornerType:(UIRectCorner)rectCornerType backgroundColor:(UIColor *)backgroundColor {
    CGSize size = self.bounds.size;
    CGFloat scale = [UIScreen mainScreen].scale;
    CGSize cornerRadii = CGSizeMake(cornerRadius, cornerRadius);
...
- (void)wvr_cornerRadiusAdvance:(CGFloat)cornerRadius rectCornerType:(UIRectCorner)rectCornerType {
    ...
    if (!self.hadAddObserver) {
        [[self class] swizzleMethod:NSSelectorFromString(@"dealloc") anotherMethod:@selector(wvr_dealloc)];
...
- (void)validateFrame {
    if (self.frame.size.width == 0) {
        [self.class swizzleMethod:@selector(layoutSubviews) anotherMethod:@selector(wvr_LayoutSubviews)];
    }
}
...
- (void)wvr_LayoutSubviews {
    [self wvr_LayoutSubviews];
    if (self.isRounding) {
        [self wvr_cornerRadiusWithImage:self.image cornerRadius:self.frame.size.width/2 rectCornerType:UIRectCornerAllCorners];
    } else if (0 != self.radius && 0 != self.roundingCorners && nil != self.image) {
        [self wvr_cornerRadiusWithImage:self.image cornerRadius:self.radius rectCornerType:self.roundingCorners];
    }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if ([keyPath isEqualToString:@"image"]) {
        ...
        [self validateFrame];
        if (self.isRounding) {
            [self wvr_cornerRadiusWithImage:newImage cornerRadius:self.frame.size.width/2 rectCornerType:UIRectCornerAllCorners];
        } else if (0 != self.radius && 0 != self.roundingCorners && nil != self.image) {
            [self wvr_cornerRadiusWithImage:newImage cornerRadius:self.radius rectCornerType:self.roundingCorners];
        }
    }
}
